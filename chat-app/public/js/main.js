...
const state = {
    ...
    rooms: [1],           // Список всех доступных комнат
    currentRoom: null,    // Текущая активная комната
    members: [1],         // Участники текущей комнаты
    onlineIds: new Set(), // Множество ID пользователей онлайн
}

// --- Логика страницы авторизации (login.html) ---
...
if (authForm) {
    ...
    authForm.addEventListener('submit', async (e) => {
        ...
        try {
            // Если выбрана регистрация - сначала создаем учетную запись
            if (ui.currentTab === 'register') {
                const name = document.getElementById('input-name').value.trim()
                await api.register(email, password, name)
            }

            // Выполняем вход и сохраняем токен
...
if (roomsList) {
    ...
    if (!token) {
        window.location.href = '/login.html' // Нет токена - на логин
    } else {
        initChat() // Запуск приложения
    }
}
...
async function initChat() {
    ...
    socketClient.onMessage((msg) => {
        // Показываем сообщение только если оно из текущей комнаты
    ...
    // Создание новой комнаты (модальное окно)
    ...
    document.getElementById('btn-confirm-room').addEventListener('click', async () => {
        ...
        try {
            ...
            state.rooms.unshift(room) // Добавляем в начало списка
            ui.renderRooms(state.rooms, state.currentRoom?.id, selectRoom)
            ui.hideModal()
        } catch (err) {
    ...
    // Присоединение к комнате (кнопка "Присоединиться")
...
// Загрузка списка комнат и восстановление последней активной комнаты
async function loadRooms() {
    const [rooms, myRoomIds] = await Promise.all([api.getRooms(), api.getMyRooms()])
    state.rooms = rooms
    state.myRoomIds = new Set(myRoomIds)

    // Пытаемся восстановить последнюю открытую комнату из localStorage
...
// Выбор комнаты (клик по элементу в списке)
function selectRoom(room) {
    if (state.currentRoom?.id === room.id) return

    // Очищаем состояние текущей комнаты, если она была
    if (state.currentRoom) {
        state.currentRoom = null
        state.members = []
        state.onlineIds = new Set()
    ...
    // Если пользователь уже участник - присоединяемся, иначе показываем приглашение
...
async function joinRoom(room) {
    ...
    ui.renderMembers([], new Set())
    socketClient.joinRoom(room.id) // Уведомляем сервер через сокет
...
function sendMessage() {
    ...
    socketClient.sendMessage(state.currentRoom.id, content) // Отправляем через сокет
