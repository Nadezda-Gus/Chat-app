// Объект для управления интерфейсом пользователя
const ui = {
    currentTab: 'login', // Текущая активная вкладка (login/register)
    ...
    renderRooms(rooms, currentRoomId, onSelect) {
        ...
        rooms.forEach(room => {
            ...
            li.className = `flex items-center gap-2 px-1.5 rounded cursor-pointer text-sm transition-colors ${isActive ? 'bg-[#484249] text-white' : 'text-[#949ba4] hover:bg-[#3537c] hover:text-white'
                }`
    ...
    // Отображение всех сообщений в комнате
    renderMessages(messages, currentUserId) {
        const area = document.getElementById('messages-area')
        // Удаляем старые сообщения, кроме служебных
    ...
    createMessageEl(msg, currentUserId) {
        const isOwn = msg.senderId === currentUserId // Своё или чужое сообщение
...
`
    ...
    // Отображение участников комнаты с индикацией онлайн
...
<div class="w-7 h-7 rounded-full bg-[#5856f2] flex items-center justify-center text-white text-xs font-bold">
 ${(user.name || user.email)[0].toUpperCase()}
  ...
  <span class="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-[#2b2d31] ${isOnline ? 'bg-green-500' : 'bg-[#747f8d]'
   ...
   `
    ...
    // Установка заголовка текущей комнаты
    setRoomHeader(room) {
    ...
    // Включение/отключение поля ввода сообщения
    ...
    // Отображение состояния "нет сообщений"
    showEmptyState() {
        document.getElementById('empty-state').classList.remove('hidden')
        document.getElementById('join-state').classList.add('hidden')
    },

    // Отображение приглашения присоединиться к комнате
    showJoinState(room) {
    ...
    // Скрытие модального окна создания комнаты
    hideModal() {
    ...
    // Установка данных текущего пользователя в UI
    setUser(user) {
